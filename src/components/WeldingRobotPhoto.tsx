...
export default function WeldingRobotPhoto() {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem("samk_welding_uploaded_photo");
      if (stored) {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        if (ctx) {
          ...
          try {
            localStorage.setItem("samk_welding_uploaded_photo", compressedDataUrl);
  ...
  const resetPhoto = (e: React.MouseEvent) => {
    ...
    if (window.confirm("Are you sure you want to delete this photo?")) {
      localStorage.removeItem("samk_welding_uploaded_photo");
      ...
      >
        ...
        {isLoading ? (
          <div className="flex flex-col items-center gap-3 font-mono text-xs text-white/60 z-20">
            ...
            <span>PROCESSING WELDING PHOTO...</span>
          </div>
        ) : uploadedImage ? (
          <div className="relative w-full h-full">
            <img 
              src={uploadedImage} 
              alt="Uploaded Welding Robot Cell" 
          ...
          <div className="flex flex-col items-center justify-center text-center p-6 z-20 font-mono select-none">
            ...
            <p className="text-[11px] text-white/50 max-w-[280px] leading-relaxed mb-3">
              Drag and drop your Han's Robot welding photo here, or click to browse from your device.
            </p>
          </div>
        )}
      </div>
